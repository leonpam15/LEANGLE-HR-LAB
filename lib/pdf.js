...
export async function generatePDF(quizTitle, primaryStyle, reportContent, leaderName = '') {
  return new Promise((resolve, reject) => {
    ...
    // PAGE: LEADERSHIP PROFILE (FIXED OVERLAP)
    doc.addPage();
    pageNum++;
    header();
    y = 55;
    ...
    // PAGE: STYLE BREAKDOWN (FIXED OVERLAP)
    doc.addPage();
    pageNum++;
    header();
    y = 55;
    ...
    styles.forEach(s => {
      ...
      y += styleDescHeight + 18;
    });

    // PAGE: 90-DAY ROADMAP (DOUBLE SPACED)
    doc.addPage();
    pageNum++;
    header();
    y = 55;
    ...
    // PAGE: 30-DAY ACTION PLAN (ON ITS OWN PAGE)
    doc.addPage();
    pageNum++;
    header();
    y = 55;
