...
export async function generatePDF(quizTitle, primaryStyle, reportContent, leaderName = '') {
  return new Promise((resolve, reject) => {
    ...
    const header = () => {
      ...
      doc.fontSize(8).font('Helvetica').fillColor('#999').text(`${pageNum}`, M + CW - 20, 15, { width: 20, align: 'right' });
    };
    ...
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('YOUR LEADERSHIP NARRATIVE', M, y);
    y += 18;
...
Many leaders go through their entire careers without truly understanding their leadership profile. They react to situations rather than lead from a place of conscious choice. This assessment changes that. It moves you from unconscious competence to conscious mastery. It gives you a framework for understanding not just what you do well, but why it matters and how to leverage it strategically.`;
    ...
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('WHAT THIS ASSESSMENT MEASURES', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
...
Each dimension represents a distinct capability that exceptional leaders develop and integrate. They work together like an orchestra - when all five are in harmony, leaders create extraordinary results. When one is significantly out of balance, it can limit overall effectiveness. The goal isn't to be high on all five - it's to understand your unique combination and leverage it strategically.`;
    ...
    dimensions.forEach(d => {
      ...
      y += 12;
      let dHeight = doc.heightOfString(d.desc, { width: CW - 15, font: 'Helvetica', size: 9 });
      doc.fontSize(9).font('Helvetica').fillColor('#666').text(d.desc, M + 12, y, { width: CW - 24 });
      y += dHeight + 14;
    });
    ...
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('LEADERSHIP EFFECTIVENESS FRAMEWORK', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
...
The framework below shows how each dimension connects to real business outcomes that matter to your organization:`;
    ...
    outcomes.forEach(o => {
      ...
      y += 11;
      let oHeight = doc.heightOfString(o.desc, { width: CW - 15, font: 'Helvetica', size: 9 });
      doc.fontSize(9).font('Helvetica').fillColor('#666').text(o.desc, M + 12, y, { width: CW - 24 });
      y += oHeight + 14;
    });
    ...
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('YOUR STRENGTHS IN ACTION', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
    ...
    scenarios.forEach(s => {
      ...
      y += 9;
      let sitHeight = doc.heightOfString(s.situation, { width: CW - 12, font: 'Helvetica', size: 9 });
      doc.fontSize(9).font('Helvetica').fillColor('#666').text(s.situation, M + 10, y, { width: CW - 20 });
      y += sitHeight + 10;
      
      doc.fontSize(9).font('Helvetica-Bold').fillColor(NAVY).text('Your Strength:', M, y);
      y += 9;
      let strHeight = doc.heightOfString(s.yourStrength, { width: CW - 12, font: 'Helvetica', size: 9 });
      doc.fontSize(9).font('Helvetica').fillColor('#666').text(s.yourStrength, M + 10, y, { width: CW - 20 });
      y += strHeight + 10;
      
      doc.fontSize(9).font('Helvetica-Bold').fillColor(NAVY).text('Your Action:', M, y);
      y += 9;
      let actHeight = doc.heightOfString(s.action, { width: CW - 12, font: 'Helvetica', size: 9 });
      doc.fontSize(9).font('Helvetica').fillColor('#666').text(s.action, M + 10, y, { width: CW - 20 });
      y += actHeight + 16;
    });
    ...
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('YOUR LEADERSHIP PROFILE', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;

    let introHeight = doc.heightOfString(`These five dimensions represent your core leadership strengths and developmental opportunities. Your profile reveals a leader who integrates multiple leadership capabilities to drive results while maintaining team engagement and organizational health. Understanding how these dimensions interact is essential to your continued development and increased impact.

Your profile isn't fixed - it's a starting point for growth. Leaders can develop in any dimension through deliberate practice, coaching, and experience. The goal isn't to be equally high on all five. The goal is to understand your unique profile and leverage it strategically while intentionally developing areas that matter for your role and aspirations.`, { width: CW, font: 'Helvetica', size: 10 });
    doc.fontSize(10).font('Helvetica').fillColor('#333').text(`These five dimensions represent your core leadership strengths and developmental opportunities. Your profile reveals a leader who integrates multiple leadership capabilities to drive results while maintaining team engagement and organizational health. Understanding how these dimensions interact is essential to your continued development and increased impact.

Your profile isn't fixed - it's a starting point for growth. Leaders can develop in any dimension through deliberate practice, coaching, and experience. The goal isn't to be equally high on all five. The goal is to understand your unique profile and leverage it strategically while intentionally developing areas that matter for your role and aspirations.`, M, y, { width: CW });
    y += introHeight + 16;

    doc.fontSize(10).font('Helvetica-Bold').fillColor(STEEL_BLUE).text('Understanding Your Scores:', M, y);
    y += 12;
    let explHeight = doc.heightOfString(`High scores (7-10): These are your natural strengths where you lead with energy and effectiveness. These are areas where you don't have to work hard to be good - they come naturally. These are the dimensions to lean into and leverage.

Medium scores (4-6): These are neutral areas. You have capability here but it doesn't come naturally. You can develop these areas through deliberate practice and coaching. They often represent your biggest growth opportunities.

Lower scores (0-3): These indicate areas to develop intentionally. They may not come naturally, but they're learnable. Depending on your role, these may be critical development priorities.

There are no "good" or "bad" scores - they simply reveal your natural leadership pattern. The key is understanding your profile and using that self-awareness to lead more effectively.`, { width: CW, font: 'Helvetica', size: 10 });
    doc.fontSize(10).font('Helvetica').fillColor('#333').text(`High scores (7-10): These are your natural strengths where you lead with energy and effectiveness. These are areas where you don't have to work hard to be good - they come naturally. These are the dimensions to lean into and leverage.

Medium scores (4-6): These are neutral areas. You have capability here but it doesn't come naturally. You can develop these areas through deliberate practice and coaching. They often represent your biggest growth opportunities.

Lower scores (0-3): These indicate areas to develop intentionally. They may not come naturally, but they're learnable. Depending on your role, these may be critical development priorities.

There are no "good" or "bad" scores - they simply reveal your natural leadership pattern. The key is understanding your profile and using that self-awareness to lead more effectively.`, M, y, { width: CW });
    y += explHeight + 16;

    doc.fontSize(10).font('Helvetica-Bold').fillColor(STEEL_BLUE).text('Your Five Dimension Scores:', M, y);
    y += 12;

    const scores = [
      { label: 'People Focus', value: 8, desc: 'You prioritize team development and emotional wellbeing as central to your leadership. This strength enables you to build loyal, engaged teams and create cultures where people do their best work. People trust you because they feel genuinely valued.' },
      { label: 'Visibility Drive', value: 7, desc: 'You are comfortable with visibility and influence in your organization. You can advocate effectively for your ideas and lead from the front. You build networks and make yourself known to decision-makers.' },
      { label: 'Speed to Act', value: 8, desc: 'You are decisive and action-oriented. You don\'t get paralyzed by analysis. You move ideas into reality quickly and energize teams with momentum and forward progress. You create a culture of execution.' },
      { label: 'Influence', value: 7, desc: 'You persuade and mobilize others with relative ease. Your ideas gain traction because people believe in your judgment and direction. You build coalitions and gain buy-in across organizational boundaries.' },
      { label: 'Innovation', value: 8, desc: 'You are forward-thinking and comfortable with change and ambiguity. You see possibilities others miss and drive transformation. You attract change-ready talent and build cultures of continuous improvement.' },
    ];
    ...
    scores.forEach(s => {
      ...
      doc.fontSize(9).font('Helvetica-Bold').fillColor(STEEL_BLUE).text(s.value + '/10', M + 255, y);
      y += 12;
      let descHeight = doc.heightOfString(s.desc, { width: CW - 130, font: 'Helvetica', size: 9 });
      doc.fontSize(9).font('Helvetica').fillColor('#666').text(s.desc, M + 130, y, { width: CW - 140 });
      y += descHeight + 14;
    });
    ...
    header();
    y = 40;
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('LEADERSHIP STYLE ANALYSIS', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
    ...
    y += 13;
    let styleIntroHeight = doc.heightOfString(`This is your natural default way of leading. It reflects how you show up when you are at your best and when under pressure. Your ${primaryStyle} style means you focus on possibilities, inspire others with compelling vision, and lead through genuine relationships. This style is particularly effective when teams need direction, clarity, and motivation during times of change or uncertainty. Your ${primaryStyle} approach creates organizational cultures where people feel inspired to do meaningful work.`, { width: CW, font: 'Helvetica', size: 10 });
    doc.fontSize(10).font('Helvetica').fillColor('#666').text(`This is your natural default way of leading. It reflects how you show up when you are at your best and when under pressure. Your ${primaryStyle} style means you focus on possibilities, inspire others with compelling vision, and lead through genuine relationships. This style is particularly effective when teams need direction, clarity, and motivation during times of change or uncertainty. Your ${primaryStyle} approach creates organizational cultures where people feel inspired to do meaningful work.`, M, y, { width: CW });
    y += styleIntroHeight + 14;

    doc.fontSize(10).font('Helvetica-Bold').fillColor(STEEL_BLUE).text('How Your Style Shows Up:', M, y);
    y += 12;
    let showsUpHeight = doc.heightOfString(`You naturally see the bigger picture and help others understand how their work contributes to larger goals. You build strong relationships quickly and use genuine connection as a primary leadership tool. You inspire others with your optimism and possibility-focused thinking. You adapt your approach based on people's emotional states and readiness for change. You enjoy building consensus but can move forward decisively when needed. Your presence creates an environment where people feel valued and motivated to do their best work. You're the kind of leader people want to work for and follow through difficult transitions.`, { width: CW, font: 'Helvetica', size: 10 });
    doc.fontSize(10).font('Helvetica').fillColor('#666').text(`You naturally see the bigger picture and help others understand how their work contributes to larger goals. You build strong relationships quickly and use genuine connection as a primary leadership tool. You inspire others with your optimism and possibility-focused thinking. You adapt your approach based on people's emotional states and readiness for change. You enjoy building consensus but can move forward decisively when needed. Your presence creates an environment where people feel valued and motivated to do their best work. You're the kind of leader people want to work for and follow through difficult transitions.`, M, y, { width: CW });
    y += showsUpHeight + 14;
    ...
    const styles = [
      { name: 'Visionary', pct: 90, desc: 'Big-picture thinking, compelling direction' },
      { name: 'Analyzer', pct: 75, desc: 'Data-driven, thorough decision-making' },
      { name: 'Coach', pct: 80, desc: 'Develop others, high emotional intelligence' },
      { name: 'Driver', pct: 70, desc: 'Results-focused, competitive, high standards' },
      { name: 'Supporter', pct: 65, desc: 'Collaborative, team player, harmony focused' },
    ];
    ...
    styles.forEach(s => {
      ...
      doc.fontSize(10).font('Helvetica').fillColor('#333').text(s.name, M, y);
      y += 11;
      ...
      doc.fontSize(9).font('Helvetica-Bold').fillColor(STEEL_BLUE).text(s.pct + '%', M + 255, y - 1);
      y += 10;
      let styleDescHeight = doc.heightOfString(s.desc, { width: CW - 110, font: 'Helvetica', size: 9 });
      doc.fontSize(9).font('Helvetica').fillColor('#666').text(s.desc, M + 110, y, { width: CW - 120 });
      y += styleDescHeight + 12;
    });
    ...
    header();
    y = 40;
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('90-DAY LEADERSHIP ROADMAP', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;

    let roadmapIntroHeight = doc.heightOfString('This roadmap breaks your development journey into three focused phases spanning 90 days. Each phase has clear milestones and builds intentionally on the previous one. The goal is to move from awareness to action to integration, creating lasting behavioral change. Track your progress weekly and celebrate wins.', { width: CW, font: 'Helvetica', size: 10 });
    doc.fontSize(10).font('Helvetica').fillColor('#666').text('This roadmap breaks your development journey into three focused phases spanning 90 days. Each phase has clear milestones and builds intentionally on the previous one. The goal is to move from awareness to action to integration, creating lasting behavioral change. Track your progress weekly and celebrate wins.', M, y, { width: CW });
    y += roadmapIntroHeight + 18;
    ...
    roadmap.forEach(r => {
      ...
      y += 14;
      doc.fontSize(9).font('Helvetica').fillColor('#999').text(r.time + ' | ' + r.overview, M + 12, y);
      y += 11;
      r.actions.forEach(a => {
        let aHeight = doc.heightOfString(a, { width: CW - 30, font: 'Helvetica', size: 9 });
        doc.fontSize(9).font('Helvetica').fillColor('#333').text('• ' + a, M + 12, y, { width: CW - 30 });
        y += aHeight + 8;
      });
      y += 14;
    });
    ...
    header();
    y = 40;
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('RECOMMENDED RESOURCES', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
    ...
    y += 11;
    let book2Height = doc.heightOfString('A classic on disruption and innovation. Provides frameworks for anticipating market shifts and leading transformation. Perfect for your Visionary strength.', { width: CW, font: 'Helvetica', size: 10 });
    doc.fontSize(10).font('Helvetica').fillColor('#666').text('A classic on disruption and innovation. Provides frameworks for anticipating market shifts and leading transformation. Perfect for your Visionary strength.', M, y, { width: CW });
    y += book2Height + 14;
    ...
    header();
    y = 40;
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('MY 30-DAY ACTION PLAN', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
    ...
    header();
    y = 40;
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('MY ACTION COMMITMENTS', M, y);
    y += 18;
    ...
    header();
    y = 40;
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('WEEKLY MICRO-HABITS', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 120, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
    ...
    header();
    y = 40;
    doc.fontSize(13).font('Helvetica-Bold').fillColor(NAVY).text('ACCOUNTABILITY PARTNER AGREEMENT', M, y);
    y += 18;
    doc.moveTo(M, y).lineTo(M + 140, y).strokeColor(GOLD).lineWidth(1.5).stroke();
    y += 20;
