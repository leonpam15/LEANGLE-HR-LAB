...
const STRESS_PROFILES = { Visionary: [['Under Pressure', 'Becomes more visionary but may lose sight of practical constraints', C.gold], ['High Stakes', 'Thrives on big challenges but struggles with small details', C.steel], ['Ambiguity', 'Energized by uncertainty and creates direction', C.green]], Recognition: [['Under Pressure', 'Doubles down on relationship-building', C.gold], ['High Stakes', 'Brings team together through recognition', C.steel], ['Ambiguity', 'Seeks clarity through dialogue', C.green]], default: [['Under Pressure', 'Adapts based on context and feedback', C.gold], ['High Stakes', 'Focuses on core priorities', C.steel], ['Ambiguity', 'Builds frameworks for structure', C.green]] };

const HOW_OTHERS = { Visionary: [['Direct Reports', 'See you as inspiring but sometimes impatient leader', C.gold], ['Peers', 'View you as strategic and forward-thinking', C.steel], ['Leadership', 'Recognize your ability to chart new territory', C.green]], Recognition: [['Direct Reports', 'Feel genuinely seen and valued', C.gold], ['Peers', 'Appreciate your ability to bring out the best', C.steel], ['Leadership', 'Value your culture-building impact', C.green]], default: [['Direct Reports', 'Appreciate your clarity and structure', C.gold], ['Peers', 'Respect your consistency and reliability', C.steel], ['Leadership', 'Value your execution and results', C.green]] };

const TEAM_COMPAT = { default: [['Works Best With', 'Leaders who complement your style', C.gold], ['Potential Friction', 'Opposite styles may create tension', C.steel], ['Growth Opportunity', 'Develop appreciation for diverse approaches', C.green]] };

const ROADMAP = { default: [['Week 1-4', C.gold, 'Build self-awareness through reflection. Notice your triggers.'], ['Week 5-8', C.steel, 'Experiment with one new behavior. Small changes compound.'], ['Week 9-12', C.green, 'Solidify the change. Make it automatic.']] };

const BOOKS = { Visionary: [['The Innovators Dilemma', 'Clayton Christensen', 'Why visionary leaders must build structures that survive disruption.'], ['Multipliers', 'Liz Wiseman', 'How the best leaders amplify intelligence.'], ['The Hard Thing About Hard Things', 'Ben Horowitz', 'When visionary leaders grind through execution.']], Recognition: [['Dare to Lead', 'Brene Brown', 'Vulnerability and care as leadership superpowers.'], ['Multipliers', 'Liz Wiseman', 'Making people feel genuinely seen.'], ['Radical Candor', 'Kim Scott', 'Combining care with direct challenge.']], default: [['Drive', 'Daniel Pink', 'The science of motivation.'], ['Essentialism', 'Greg McKeown', 'Doing less but better.'], ['Deep Work', 'Cal Newport', 'Excellence through focused effort.']] };

const BRAND_STATEMENTS = { Visionary: (name, first) => `${name} is a Visionary leader who transforms organisations by seeing possibilities before others can articulate them. Known for reframing complex problems, inspiring teams with compelling future direction, and building strategic clarity from ambiguity, ${first} leads from the conviction that the most valuable thing a leader can offer is a vision worth following.`, Executor: (name, first) => `${name} is an Executor leader who creates high-performing teams by turning strategy into results with relentless precision. Known for driving accountability, delivering under pressure, and building cultures where commitments are kept, ${first} leads from the belief that great leadership is ultimately measured by what gets done.`, Recognition: (name, first) => `${name} is a Recognition leader who creates high-performing cultures by making people feel genuinely seen and valued. Known for transforming disengaged teams, delivering results with energy and precision, and building environments where talent wants to stay, ${first} leads from the belief that recognition is not a reward—it is a strategy.`, default: (name, first, style) => `${name} is a ${style} leader who creates high-performing teams through their distinctive approach. Known for bringing clarity, energy, and genuine care to every team they lead, ${first} leads from a deep belief that great leadership starts with self-awareness.` };
...
export async function generatePDF(quizTitle, primaryStyle, reportContent, leaderName = '') {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(9.5).font('Helvetica-Bold').fillColor(C.navy).text("WHAT'S INSIDE THIS REPORT", M, cp(0.65), { align: 'center', width: CW });
    const iL = ["Leadership DNA", "3 Superpowers", "Growth Edge", "30-Day Plan", "Stress Profile"];
    const iR = ["Others Experience You", "Team Compatibility", "90-Day Roadmap", "Leadership Brand", "Certificate"];
    ...
    doc.fontSize(9).font('Helvetica-Bold').fillColor(C.white).text('Your Leadership Report', M, 42, { align: 'center', width: CW });
    ...
    // Brand Statement
    y = addPage();
    heading('Leadership Brand Statement', 'Use in interviews and LinkedIn');
    ...
    // Stress
    needSpace(60);
    heading('Your Stress & Pressure Profile', 'How you adapt under pressure');
    stressProfile.forEach(([label, body, col]) => { infoBox(label, body, col); y += 2; });

    // Others Experience
    needSpace(60);
    heading('How Others Experience You', 'Direct reports, peers, and leadership');
    ...
    // Team Compat
    needSpace(60);
    heading('Team Compatibility Guide', 'Working with different styles');
    teamCompat.forEach(([label, body, col]) => { infoBox(label, body, col); y += 2; });

    // Roadmap
    needSpace(60);
    heading('90-Day Growth Roadmap', 'Three months of intentional practice');
    roadmap.forEach(([title, col, body]) => { roadmapBox(title, col, body); y += 2; });

    // Books
    needSpace(60);
    heading('Recommended Reading', 'Books for your growth');
    ...
    doc.fontSize(11).font('Helvetica').fillColor(C.muted).text('has completed the', M, cm + 24, { align: 'center', width: CW });
    ...
    const assessments = ['Communication', 'Conflict', 'Feedback', 'Decision-Making', 'Motivation', 'Stress', 'Personality'];
    ...
    doc.fontSize(8).font('Helvetica').fillColor(C.white).text('© 2025 LEANGLE HR LAB', M, 14, { align: 'center', width: CW });
