...
export async function generatePDF(quizTitle, primaryStyle, reportContent, leaderName = '') {
  return new Promise((resolve, reject) => {
    ...
    for (const section of sections) {
      ...
      for (const line of section.content) {
        ...
        if (line.startsWith('-')) {
          ...
          y += doc.heightOfString('• ' + line.replace(/^-\s+/, ''), { width: CW - 30, font: 'Helvetica', size: 11 }) + 10;
        } else {
          doc.fontSize(11).font('Helvetica').fillColor('#333').text(line, M, y, { width: CW });
          y += doc.heightOfString(line, { width: CW, font: 'Helvetica', size: 11 }) + 10;
        }
      }
      y += 18;
    }
    ...
    y += 12;
    doc.fontSize(11).font('Helvetica').fillColor('#666').text('Your combination of People Focus and Speed to Act means you\'re effective at both employee engagement and driving results.', M, y, { width: CW });
